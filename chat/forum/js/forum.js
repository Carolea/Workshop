// Si vous avez remarqué dans le fichier get-message.php, on peut voir qu'en cliquant sur le lien du pseudo, on a une fonction qui se déclenche : insertLogin().
 // Celle-ci permet d'insérer le pseudo dans la zone de texte. Voici donc cette merveilleuse fonction. ;)
	...
	$.getJSON('phpscripts/get-online.php', function(data) {
		...
		if(data['error'] == '0') {
			...
			for (var id in data['list']) {
				...
				online += '<img src="img/status-'+image+'.svg" /> ';
...
// Il nous suffira de mettre à jour le statut lors de la modification du champ de sélection (d'où l'attribut onchange sur le <select>).
