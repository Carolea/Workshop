...
$db = db_connect();
?>
<!-- Ici, nous allons vérifier que le compte de l'utilisateur existe toujours.
S'il a été supprimé, il ne faut pas le laisser écrire des messages et on doit le rediriger après avoir supprimé ses sessions.
La redirection s'effectuera en Javascript. Le fichier renvoie 0 si le compte a été supprimé et les messages dans le cas contraire. -->
...
?>
Nous devons ensuite afficher l'annonce, située en haut dans le chat.
Extrêmement simple, nous ne feront qu'afficher le message présent dans la table chat_annonce.
Pour rappel, nous utilisons la librairie PDO pour effectuer des interactions avec la base de données.
Les autres codes qui vont suivre devront se trouver à la place du commentaire "ON PEUT CONTINUER !!!" du code précédent.
